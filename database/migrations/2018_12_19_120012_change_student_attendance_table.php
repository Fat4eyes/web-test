...
class ChangeStudentAttendanceTable extends Migration
{
    public function up()
    {
        Schema::table('student_attendance', function (Blueprint $table) {
            $table->dropForeign('student_attendance_discipline_group_id_foreign');
            $table->integer('discipline_plan_id')->unsigned()->nullable()->index('student_attendance_discipline_plan_id_foreign');
            $table->dropColumn('discipline_group_id');
        });

        Schema::table('student_attendance', function (Blueprint $table) {
            $table->foreign('discipline_plan_id', 'student_attendance_discipline_plan_id_foreign')
